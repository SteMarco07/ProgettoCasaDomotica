...
public class Main {
    ...
    public static int gestioneSingolaLampadina(SistemaDomotico sistemaDomotico, Scanner in, String nomeStanza) {
        if (sistemaDomotico.getNumLampadineStanza(nomeStanza) != 0) {
            ...
            String nomePresa = in.nextLine();
            try {
                Presa presa = sistemaDomotico.getStanza(nomeStanza).getPresa(nomePresa);
                if (presa.getLampadina() == null) {
                    System.out.println("La presa " + nomePresa + " non ha una lampadina collegata");
                    return -1;
                }
                String nomeLampadina = presa.getLampadina().getNome();
                while (true) {
                ...
                System.out.println("La lampadina nella presa " + nomePresa + " non è stata trovata");
                return -1;
            }
        } else {
